...
const MASS = [
  ...
  ['brow', 0.34, 1.70, 0.55, 0.95, -0.05, 0.76, 0.05, 0.16],
  ['crown', 0.42, 1.35, 0.75, 1.10, 0.20, 0.88, -0.55, 0.06],
  ...
  ['boulder', 0.20, 1.15, 0.75, 1.15, -1.35, 0.05, 0.62, 0.19],
  ['boulder', 0.15, 1.00, 0.70, 1.00, 1.30, 0.04, 0.55, 0.08],
];
...
export function makeCave({
  ...
  const throat = new THREE.Mesh(
    new THREE.BoxGeometry(width * 1.30, height * 0.80, depth * 1.35),
    toonMaterial({ color: mixHex(color, INK, 0.93), flat: true }));
  throat.name = 'throat';
  throat.position.set(0, height * 0.30, -depth * 0.42);
  g.add(throat);
