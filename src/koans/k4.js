...
export default {
  ...
  build(ctx) {
    ...
    // The painted Bodhidharma: a hooded mass and a bare face, in ink, standing
    // a hair proud of the silk. Famously bearded everywhere else; not here.
    const inkMat = toonMaterial({ color: INK, flat: true });
    ...
    const robe = new THREE.Mesh(new THREE.LatheGeometry(robeProfile, 9), inkMat);
    robe.name = 'robe';
    robe.position.y = -0.62;
    robe.scale.z = 0.30;                 // pressed flat: it is paint, not a man
    painted.add(robe);

    const face = new THREE.Mesh(new THREE.SphereGeometry(0.135, 12, 9), inkMat);
    face.name = 'face';
    face.position.y = 0.30;
    face.scale.z = 0.32;
    painted.add(face);

    const hood = new THREE.Mesh(new THREE.SphereGeometry(0.175, 12, 8, 0, Math.PI * 2, 0, Math.PI * 0.62), inkMat);
    hood.name = 'hood';
    ...
    // the collector's seal, bottom corner — the one warm mark, where a seal
    // belongs on a hanging scroll
    const seal = new THREE.Mesh(
      new THREE.BoxGeometry(0.13, 0.13, 0.012),
      toonMaterial({ color: ACCENT, flat: true }));
    seal.name = 'seal';
    seal.position.set(-SW / 2 + 0.22, -SH / 2 + 0.30, 0.02);
    scroll.add(seal);
