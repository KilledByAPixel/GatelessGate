...
const CAM = { distance: 17, target: [-0.3, 2.8, -1.6], heading: 31.5, pitch: 14.4 };
  ...
  // THE PRESENT: a seam between the last two ages, thinner than any of them — a
  // slightly darker band, not the seal. The accent belongs to the Buddha here:
  // the figure is what the case is about, not the ground.
  if (i === BANDS.length - 2) {
  ...
  //
  // OFF TO THE SIDE. It used to sit at (-0.9, 2.75), which from the home lens
  // is the same screen column the colossus occupies — a red-brown smudge parked
  // in front of the one thing the case is about. Now it sits at the frame's
  ...
  // is what "an octave under" was reaching for with a raw f0 — task-12's
  // migration to the tuned presets picks `great` outright.
  //
  // And then bigger still. `great` at its own 2.36 was not enough for a Buddha
  // of this size — a giant's bell wants to be deeper AND longer — so the
  // preset's tuned voice is cast at a larger BODY instead: f0 drops about six
  // semitones and every mode's decay grows with it, one number doing both
  // halves at once. The tuned amplitudes are untouched. ...and struck harder
  // than anything else in the book. A big bell is not just a low one, and it
  // was still hard to hear: the size increase alone made it deeper without
  // making it arrive: casting the voice bigger spreads the same energy over
  // more, longer-decaying modes, so the onset actually got quieter. gain is a
  // plain strike velocity here, so this is how hard the beam swings.
