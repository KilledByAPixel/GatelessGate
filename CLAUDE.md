...
- **The spine** (`src/spine.js`): the book's actual reading order — preface, the forty-nine in id order, afterword — is a list of slugs, not `CASES`; `neighborSlug` walks it for paging and the arrow keys, and it's the one place that knows front/back matter sit either side of the numbers. **Matter pages** (`src/koans/matter/preface.js`, `afterword.js`) are koan-shaped modules that differ from a case only in `id: null`, which is what turns off the numeral seal and the narration controls; `registry.js` keys them by slug in their own `MATTER_LOADERS` table, separate from the numeric `LOADERS`. Their text is generated by `scripts/build-text.js` from a second, independent input, `local/mumonkan-front-and-back-matter.md`, into `src/koans/text/matter.js` — `mumonkan.js` and its 147 narration bakes are never touched by that path.
