...
| Narration path | M1: `speechSynthesis` (Chrome-first; Firefox default voice accepted for the first pass). M2: pre-baked per-case Opus generated locally with Piper, lazy-loaded behind the same `speak()` interface. Runtime WASM TTS rejected (model weight) | Frank + AI-review round, 2026-07-17 |
