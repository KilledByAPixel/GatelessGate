...
export function makeMonk({ height = 1.6, stout = 1, color = INK, hat = true, pose = 'stand', elder = false } = {}) {
  ...
  const profile = (seated ? sitProfile : standProfile)
    .map(([r, y], i) => new THREE.Vector2((i === 0 ? r : r * s) * height, y * height)); // index 0 stays unscaled by stout, matching the original stand/point profile
...
export function makeFlower({ height = 0.6, petals = 6, color = '#EED9E2', seed = 6 } = {}) {
  ...
  g.dropPetal = () => {
    ...
    g.remove(petal);
    petal.position.copy(world);   // correct once the caller adds it to the scene root (the held flower is transformed)
    return petal;
  };
  return g;
}
