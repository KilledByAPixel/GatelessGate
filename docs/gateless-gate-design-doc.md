...
13. *(2026-07-22)* **Sound resolved: two synthesized layers, no files.** Note 8's "maybe" is settled and the authored-track experiment is over — the Suno tracks are deleted, and no case names a track any more. Instead: **diegetic objects that sound because they exist** (first the fūrin, the glass wind bell, ringing on the wind synth's actual gust crests — one sim clock drives both the wind's gust and the chime, so they cannot drift apart), plus a **sparse non-diegetic drift** of swelled tones, weighted-random and unseeded, for the scenes with nothing in them to make a noise. The two are coupled — the drift thins automatically as a scene's emitter count rises, so a scene with a chime needs no music and a bare hillside gets the drift in full. One tuning for the whole book (hirajōshi), the way there is one ink and one accent. Still no chanting, no voice, no singing bowl. Design: `docs/superpowers/specs/2026-07-22-ambient-sound-design.md`.

14. *(2026-07-22)* **Narration reversed: baked audio files, not `speechSynthesis`.** Note 7 is superseded. The browser's voices never got past robotic, and the reading is not a side feature — the whole app is a room built around someone reading these forty-eight stories aloud, so a synthetic reader undercut every other decision in the art direction. All 147 sections (49 cases × case/comment/verse) are pre-generated with OpenAI's `gpt-4o-mini-tts` and committed as mp3, lazy-loaded per case. The voice is **`onyx`** with an instruction preset describing an elderly Japanese man reading fluent English, slowly, accent restrained — chosen by ear from a two-round audition. `speechSynthesis` and its voice-picking heuristics are deleted outright; there is no runtime voice choice any more, because there is nothing left to choose. Cost is ~$1.50 per full re-bake and ~42 MB of committed audio, which does **not** count against the ~1.5 MB app budget — that budget covers code and first load, and narration is neither. This does not disturb note 13: that settles the *ambient* layer (still synthesized, still no files, still no chanting); this is the reading voice, a separate channel that ducks the ambience while it speaks. Design: `docs/superpowers/specs/2026-07-22-baked-narration-design.md`.
...
16. *(2026-07-23)* **M3 reached: all forty-eight cases are staged.** The thirty-one cases that were still falling back to the default landscape now have dioramas of their own, built in one pass from the kit. Six new kit pieces carry the objects the remaining cases turn on — `makeWheel` (the nave comes out and the bare rim keeps turning), `makeScale` (always settles back to three pounds), `makeDrum`, `makeRack` (given, taken, given — it can never resolve), `makeBirds` and `makeSnow`. Case 49, Amban's addendum, at first kept the default landscape on purpose (the traditional extra, not a forty-ninth koan) — later staged as the closing scene, see note 17.
...
17. *(2026-07-24)* **The book has a closing scene now, and the kit grew to earn it.** A run of art passes on Frank's eye, and one new case:
    - **Case 49 is "the book at rest."** Amban's addition ends the Mumonkan on a gesture, not a word — he draws "a little circle" and says everything is contained in it. So the last case is no longer the bare default landscape: it gathers a few of the book's own living things (a pond with koi, birds crossing, the path that ran under every unstaged case) and holds them quiet, with a red **torii gate** standing at the head of the path as the one warm mark — and it is the *same* gate the title screen opens on, so the reader leaves the book through the door they entered. (It began as an ensō — Amban's "little circle" — but a hand-drawn brush ring is beyond a torus, and Frank called it for the gate, which bookends the intro and is the image the whole book is named for.) It is the only place in the book that shows more than one koan's worth of life at once — allowed because the gate keeps it disciplined: everything stays quiet around the one red mark.
    - **Water knows its container now.** `makeWater` takes a shape (round/square) and tessellates a real grid cut to it, with the ripples a height field pinned at the rim — so a stone dropped by the bank no longer throws its ring out over the grass (case 39), and a basin's water no longer pokes its square corners through round stone (7, 30, 33). New `makeBasin` gives an *open* vessel (the ponds were solid cylinders whose caps hid the water and the koi). Koi ride the surface via its height field.
    - **Birds fly.** After a false start at a full land/peck/take-off cycle (it read wrong), they were pulled back to what the two cases want: a simpler blunt-winged shape crossing the sky on drifting circuits, always aloft.
    - **`makeMonk` is the shared human plan**, the `makeQuadruped` of people — every figure selects from `hat`/`arms`/`elder`/`pose`/`stout`/`color`. Case 45 became the busy street its koan names (a `makeStall` row, keepers, a waiting customer, strollers, bystanders) with a red **horse** from the verse as its seal (`makeHorse` off the quadruped plan).
    - **Defaults, per Frank's eye:** the inverted-hull ink outlines are on by default and ink strength sits at 0.5 (the debug key bumped to v4 so a stored state can't mask the look).
