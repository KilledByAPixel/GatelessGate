...
7. *(2026-07-17)* **Narration plan tightened:** `speechSynthesis` is fine for the first pass (Chrome voices are decent); Firefox's are not — so the pre-baked per-case audio (§9's post-launch idea) moves up to **M2**, generated locally with Piper and lazy-loaded (~200–300 KB per case). Runtime in-browser neural TTS (Piper-WASM / Kokoro) is rejected: 20–300 MB model downloads against our ~1.5 MB app.
